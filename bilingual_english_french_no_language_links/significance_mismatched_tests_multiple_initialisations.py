...
for args_fn in Path('model_metadata').rglob('*/args.pkl'):
    ...
    #             _, _, secondary_query_output = english_model(secondary_query_output.to(rank))
            ...
            for i in range(query_output.size(0)):
                ...
                if model_instance not in significance_dict[ep_num][query_audio_samples[i]]: 
                    significance_dict[ep_num][query_audio_samples[i]][model_instance] = {}

                if instance in significance_dict[ep_num][query_audio_samples[i]][model_instance]: print('PROBLEM')
                significance_dict[ep_num][query_audio_samples[i]][model_instance][instance] = int(index==0)
...
for i in valid_instances:
    header.append(str(i))
trial = 1
with open('results/significance_mismatched_ME_multiple.csv', 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
