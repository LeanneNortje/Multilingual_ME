...
valid_instances = {'english':[1], 'english-dutch':[3], 'english-french':[3], 'english-dutch-french':[1]}
...
monolingual_fn = Path('../english')
bilingual_de_fn = Path('../bilingual_dutch_english_no_language_links')
bilingual_ef_fn = Path('../bilingual_english_french_no_language_links')
trilingual_fn = Path('../trilingual_no_language_links')
dirs = {
    'english': monolingual_fn,
    'english-dutch': bilingual_de_fn,
    'english-french': bilingual_ef_fn,
    'english-dutch-french': trilingual_fn
}
print(monolingual_fn.is_dir(), bilingual_de_fn.is_dir(), bilingual_ef_fn.is_dir(), trilingual_fn.is_dir())
significance_dict = {}

for language in dirs:
    directory = dirs[language]
    for args_fn in (directory / Path('model_metadata')).rglob('*/args.pkl'):
        instance = int(args_fn.parent.stem)
        if instance not in valid_instances[language]: continue
        ...
        info, epoch, global_step, best_epoch, best_acc = loadModelAttriburesAndTrainingAMP(
            directory / args["exp_dir"], english_model, image_model, attention, contrastive_loss, optimizer, 
            rank, False
            )
        ...
        with torch.no_grad():
            for ep_num in tqdm(query, desc=f'{model_instance}'):
                ...
                for i in range(query_a.size(0)):
                    ...
                    if language in significance_dict[ep_num][query_audio_samples_a[i]][model_instance]: print('PROBLEM')
                    significance_dict[ep_num][query_audio_samples_a[i]][model_instance][language] = int(index==0)
                    ...
                    if language in significance_dict[ep_num][query_audio_samples_b[i]][model_instance]: print('PROBLEM')
                    significance_dict[ep_num][query_audio_samples_b[i]][model_instance][language] = int(index==1)
...
with open('results/significance_novel_novel_tests_multiple.csv', 'w', encoding='UTF8') as f:
    ...
    for num in tqdm(significance_dict, desc=f'Writing to csv'):
        ...
        for query in significance_dict[num]:
            ...
            for language in dirs:
                row.append(entry['cpc+alexnet'][language])
            writer.writerow(row)
            trial += 1
