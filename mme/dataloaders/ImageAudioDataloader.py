...
class ImageAudioDatawithSampling(Dataset):
    ...
    def _SampleNeg(self, im_name, english_wav_name, french_wav_name, dutch_wav_name, im_path, id):
        n = self.n + 1
        negatives = []
        ...
        for i in range(n):
            ...
            temp = self.neg_masks_to_images[id]['french'][neg_ids[i]].copy()

            neg_french_fn = np.random.choice(temp, size=1)[0]
            neg_french_name = str(Path(neg_french_fn).stem)
            neg_french_fn = Path('data/french') / str(neg_french_name + '.npz')
            if french_wav_name == neg_french_name: print("Audio negative sampling went wrong.")
            french_audio_feat1 = np.load(neg_french_fn)["audio_feat"]
            french_audio_feat1, french_nframes1 = self._PadFeat(french_audio_feat1)
    ...
    def __getitem__(self, index):
        ...
        frenchpath = np.random.choice(self.pos_masks_to_images[id]['french'][french_name[0]], size=1)[0]
        frenchpath = Path('data/french') / Path(frenchpath.stem + '.npz')
        french_audio_feat = np.load(frenchpath)['audio_feat']
        ...
        for p in range(num_pos):
            ...
            pos_french_path = Path('data/french') / str(french_name[p+1] + '.npz')
            pos_french_audio_feat1 = np.load(pos_french_path)["audio_feat"]
            pos_french_audio_feat1, pos_french_nframes1 = self._PadFeat(pos_french_audio_feat1)
            ...
            positives.append({
                ...
                "pos_french": pos_french_audio_feat1, 
        ...
        negatives = self._SampleNeg(image_name[0], wav_name, french_name[0], dutch_name[0], self.data[index], id)
        
        return {"image": image, 
        "english_feat": english_audio_feat, 
        "french_feat": french_audio_feat, 
