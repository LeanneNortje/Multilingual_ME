...
class ImageAudioDatawithSampling(Dataset):
    def __init__(self, image_base_path, dataset_json_file, lookup_file, args, rank):
        ...
        self.english = args['training_languages']['english']
        self.dutch = args['training_languages']['dutch']
        self.french = args['training_languages']['french'] 
    ...
    def _SampleNeg(self, im_name, wav_names, im_path, id):
        n = self.n + 1
        negatives = []

        neg_ids = np.random.choice(list(self.neg_masks_to_images[id]['images'].keys()), size=n) #*!
        ...
        for i in range(n):
            ...
            if self.english:
                temp = self.neg_masks_to_images[id]['english'][neg_id].copy()
                ...
                if wav_names['english'] == neg_eng_name or neg_id == id: print("English audio negative sampling went wrong.")
                # data_point = np.load(neg_eng_fn)
                english_audio_feat = np.load(neg_eng_fn)["audio_feat"]
                english_audio_feat, english_nframes = self._PadFeat(english_audio_feat)
                send['neg_english'] = english_audio_feat

            temp = self.neg_masks_to_images[id]['images'][neg_id].copy()

            neg_img_fn = np.random.choice(temp, size=1)[0]
            neg_img_name = str(Path(neg_img_fn).stem)
            if im_name == neg_img_name or neg_id == id:  print("Image negative sampling went wrong.")
            ...
            if self.dutch:
                temp = self.neg_masks_to_images[id]['dutch'][neg_id].copy()
                neg_dutch_fn = np.random.choice(temp, size=1)[0]
                neg_dutch_name = Path(neg_dutch_fn).stem
                if wav_names['dutch'] == neg_dutch_name or neg_id == id: print("Dutch audio negative sampling went wrong.")
                neg_dutch_fn = Path('data/dutch') / str(neg_dutch_name + '.npz')
                dutch_audio_feat = np.load(neg_dutch_fn)['audio_feat']
                dutch_audio_feat, dutch_nframes = self._PadFeat(dutch_audio_feat)
                send['neg_dutch'] = dutch_audio_feat


            if self.french:
                temp = self.neg_masks_to_images[id]['french'][neg_id].copy()
                neg_french_fn = np.random.choice(temp, size=1)[0]
                neg_french_name = Path(neg_french_fn).stem
                if wav_names['french'] == neg_french_name or neg_id == id: print("French audio negative sampling went wrong.")
                neg_french_fn = Path('data/french') / str(neg_french_name + '.npz')
                french_audio_feat = np.load(neg_french_fn)['audio_feat']
                french_audio_feat, french_nframes = self._PadFeat(french_audio_feat)
                send['neg_french'] = french_audio_feat

            negatives.append(send)
    ...
    def __getitem__(self, index):
        ...
        if self.english:
            ...
            english_audio_feat, english_nframes = self._PadFeat(english_audio_feat)
            anchor_wav_names['english'] = str(Path(self.data[index]).stem)
            send['english_feat'] = english_audio_feat
        
        if self.dutch:
            names = list(self.pos_masks_to_images[id]['dutch'].keys())
            dutch_name = np.random.choice(names, size=num_pos+1, replace=False)
            dutch_fn = np.random.choice(self.pos_masks_to_images[id]["dutch"][dutch_name[0]], size=1)[0]
            dutch_fn = Path('data/dutch') / Path(dutch_fn.stem + '.npz')
            dutch_audio_feat = np.load(dutch_fn)['audio_feat']
            dutch_audio_feat, dutch_nframes = self._PadFeat(dutch_audio_feat)
            anchor_wav_names['dutch'] = str(Path(dutch_fn).stem)
            send['dutch_feat'] = dutch_audio_feat

        if self.french:
            names = list(self.pos_masks_to_images[id]['french'].keys())
            french_name = np.random.choice(names, size=num_pos+1, replace=False)
            french_fn = np.random.choice(self.pos_masks_to_images[id]['french'][french_name[0]], size=1)[0]
            french_fn = Path('data/french') / Path(french_fn.stem + '.npz')
            french_audio_feat = np.load(french_fn)['audio_feat']
            french_audio_feat, french_nframes = self._PadFeat(french_audio_feat)
            anchor_wav_names['french'] = str(Path(french_fn).stem)
            send['french_feat'] = french_audio_feat
        ...
        for p in range(num_pos):
            ...
            temp = {"pos_image": pos_image1}

            if self.english:
                id_lookup = self.pos_masks_to_images[id]['english'].copy()
                names = list(id_lookup.keys())
                names.remove(anchor_wav_names['english'])
                pos_wav_name = np.random.choice(names, size=1)[0]
                pos_wav_path = Path('data/english') / str(pos_wav_name + '.npz')
                pos_english_audio_feat = np.load(pos_wav_path)["audio_feat"]
                if len(pos_english_audio_feat.shape) != 2: print("positive ", p, id, '\n')
                pos_english_audio_feat, pos_english_nframes = self._PadFeat(pos_english_audio_feat)
                temp['pos_english'] = pos_english_audio_feat

            if self.dutch:
                pos_dutch_fn = np.random.choice(self.pos_masks_to_images[id]["dutch"][dutch_name[p+1]], size=1)[0]
                pos_dutch_fn = Path('data/dutch') / Path(pos_dutch_fn.stem + '.npz')
                pos_dutch_audio_feat = np.load(pos_dutch_fn)['audio_feat']
                pos_dutch_audio_feat, pos_dutch_nframes = self._PadFeat(pos_dutch_audio_feat)
                temp['pos_dutch'] = pos_dutch_audio_feat

            if self.french:
                pos_french_fn = np.random.choice(self.pos_masks_to_images[id]['french'][french_name[p+1]], size=1)[0]
                pos_french_fn = Path('data/french') / Path(pos_french_fn.stem + '.npz')
                pos_french_audio_feat = np.load(pos_french_fn)['audio_feat']
                pos_french_audio_feat, pos_french_nframes = self._PadFeat(pos_french_audio_feat)
                temp['pos_french'] = pos_french_audio_feat
        ...
        send['positives'] = positives

        negatives = self._SampleNeg(image_name[0], anchor_wav_names, self.data[index], id)
        send['negatives'] = negatives
