...
for args_fn in Path('model_metadata').rglob('*/args.pkl'):
    ...
    if instance not in valid_instances: continue
    ...
    with torch.no_grad():
        ...
        for ep_num in tqdm(query, desc=f'{model_instance}'):
            ...
            for i in range(query_output.size(0)):
                ...
                if model_instance not in significance_dict[trial]: significance_dict[trial][model_instance] = {}

                if instance in significance_dict[trial][model_instance]: print('PROBLEM')
                significance_dict[trial][model_instance][instance] = int(index==0)
                trial += 1
                ...
                if instance in significance_dict[trial][model_instance]: print('PROBLEM')
                significance_dict[trial][model_instance][instance] = int(index==1)
                trial += 1
...
for i in valid_instances:
    header.append(str(i))
with open('results/significance_familiar_multiple.csv', 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    writer.writerow(header)
    for trial in tqdm(significance_dict, desc=f'Writing to csv'):

        entry = significance_dict[trial]
        row = [trial, entry['query'], entry['image_A'], entry['image_B']]
